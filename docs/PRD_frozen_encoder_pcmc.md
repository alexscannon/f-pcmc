...
*(As built at T3, owner-approved 2026-07-10: (a) `provenance` is `Literal["initial", "seeded", "promoted"]` — the two-value list above names no value for an unpromoted STM candidate; seeding records "seeded", FR-7.1 promotion flips it to "promoted", and FR-8.3's never-merge-two-"initial" rule is unaffected. (b) Seed-count semantics: the seeding embedding counts in `ref_count_seen` (it occupies the reservoir, so FR-1.1's `K_max/ref_count_seen` needs it) but not in `match_count`/`match_windows` — FR-9 treats seeding as the no-match branch — so FR-3.3 maturity and FR-7 size counts are post-seed matches; `last_matched_at = created_at = seed step`, keeping FR-3.1 LRU defined from birth. (c) The cached κ (FR-4.2) is recomputed on every ref_set-changing observation; FR-5.1's lazy trigger governs τ recomputation only. (d) The dataclass additionally carries construction-fixed operational fields — a per-concept reservoir `np.random.Generator` (named substream) and the `window_W`/`K_max_refset`/`alpha_stm_ema` scalars — so `add_observation(z, step)` needs no per-call config; `merged_from` is the survivor-side absorbed-id list (the FR-1.4 map is the store-level view); `concept_id` reassignment/deletion raise. See docs/CHANGES.md T3.)*
...
*(As built at T4, owner-approved 2026-07-10: (a) a member's LOO vmf score is computed against the concept's cached (centroid, κ) as-is — no per-member parameter re-fit; "leave-one-out" bites only where self-exclusion matters, the knn_ref pairwise distances (self masked to +inf; duplicate rows legitimately score 0 against their twins). (b) Below the computable floors thresholds hold the prior: τ at ref_set size 1 is set to τ_prior exactly (a singleton's LOO view is empty); τ_vmf is recomputed only once ref_set ≥ n_vmf_min — below that VmfScorer is in knn_ref-fallback and never reads it, so it keeps its seeded prior. (c) FR-5.1 vs FR-5.2 select on `status`: LTM → pure percentile, STM → shrinkage, at every n. (d) The ≥25% lazy trigger reads a dirty counter of actual ref_set mutations (`Concept.refset_changes_since_tau`; post-fill skipped reservoir draws don't count) against the current ref_set size, evaluated only when `fpcmc.thresholds.maybe_recompute` is called (check-on-call; the T5 store picks the call site); every recompute resets the counter; the trigger governs τ/τ_vmf only, never κ. (e) Percentile = `np.percentile` with the default linear interpolation, mirrored from the source batch pipeline's τ calibration (`lib/evaluation/continual/paradigms/knn_vmf.py::_calibrate_tau`). FR-5.4's hook is `recompute_on_promotion(concept, config)` — pure FR-5.1 for both taus from the full ref_set, independent of the current `status` value. See docs/CHANGES.md T4.)*
...
*(As built at T5, owner-approved 2026-07-11: the cascade is `fpcmc/concepts.py::ConceptStore.route(z, step) -> RoutingResult`. (a) `RoutingResult` carries `prediction` (concept_id at tier 1; `"unknown"` at tiers 2/3), `concept_id` (assigned or seeded), `tier` (1/2/3), `score`/`margin` (the winning ScoreDetail's values — under knn_vmf the scalar is the knn_ref sub-score per the FR-4.3 note; NaN at tier 3, where nothing accepted), and `via`/`fallback` sub-scorer metadata for the event log and the A5 ablation. (b) Tier membership is computed live from `concept.status`/`match_count` at every call — this is the mechanism of FR-5.4 promotion-aware routing: a status flip plus `recompute_on_promotion` participates in tier 1 on the very next example. (c) The FR-5.1 lazy threshold check (`fpcmc.thresholds.maybe_recompute`) runs after every assignment's `add_observation`, on the matched concept only, tiers 1 and 2 alike — the sole call site. (d) FR-3.2 seeding bootstraps both taus from the frozen prior pair; concept ids are store-allocated, zero-padded per FR-1's literals (`ltm_{:03d}`/`stm_{:04d}`), unique run-wide and never reused; the allocator raises on overflow rather than widening, since the FR-4.3 lexicographic tie-break makes id ordering behavior-relevant. (e) Cross-concept scoring is batched as per-concept GEMV + vectorized composition/selection, bitwise-identical to the frozen scorers — a stacked single GEMV is measurably not (1-ulp summation-order divergence), and the identity guard `test_vectorized_matches_loop` is binding. `route()` mutates the store; read-only scoring sweeps (e.g. the T6 M1 gate) must use the scorer directly. See docs/CHANGES.md T5.)*
